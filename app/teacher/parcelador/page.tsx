...
import {
  addParceladorTema,
  deleteParceladorTema,
  getTeacherSubjects,
  parceladorTemas,
  subjects,
  updateParceladorTema,
} from "@/lib/data"
import { redirect } from "next/navigation"
import { PlusCircle, Trash2, Edit, Search, FileText } from "lucide-react"

export default function TeacherParceladorPage() {
  ...
  // Filtrar temas del profesor actual
  const teacherTemas = user ? temasList.filter((tema) => tema.profesorId === user.id) : []

  // Filtrar temas según los criterios de búsqueda
  const filteredTemas = teacherTemas.filter((tema) => {
  ...
  const handleCreateTema = () => {
    if (!newTema.titulo || !newTema.materiaId) {
      toast({
        title: "Error",
        description: "Los campos título y materia son obligatorios",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      const createdTema = addParceladorTema({
        ...newTema,
        profesorId: user!.id,
      })
  ...
  const handleUpdateTema = () => {
    if (!currentTema || !currentTema.titulo || !currentTema.materiaId) {
      toast({
        title: "Error",
        description: "Los campos título y materia son obligatorios",
        variant: "destructive",
    ...
    try {
      const updatedTema = updateParceladorTema(currentTema.id, {
        titulo: currentTema.titulo,
        descripcion: currentTema.descripcion,
        objetivos: currentTema.objetivos,
        contenido: currentTema.contenido,
        actividades: currentTema.actividades,
        recursos: currentTema.recursos,
        evaluacion: currentTema.evaluacion,
        fechaInicio: currentTema.fechaInicio,
        fechaFin: currentTema.fechaFin,
        materiaId: currentTema.materiaId,
        grado: currentTema.grado,
        grupo: currentTema.grupo,
      })
      ...
      if (updatedTema) {
        ...
        toast({
          ...
          description: `El tema "${updatedTema.titulo}" ha sido actualizado correctamente`,
        })
  ...
  return (
    <div className="container py-10 app-background">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-secondary">Mi Parcelador de Temas</h1>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Modifica los datos del tema seleccionado.</DialogDescription>
          </DialogHeader>
          {currentTema && (
            <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto">
              <div className="grid gap-2">
                ...
                <Input
                  id="edit-titulo"
                  value={currentTema.titulo}
                  onChange={(e) => setCurrentTema({ ...currentTema, titulo: e.target.value })}
                />
              </div>
              <div className="grid gap-2">
                ...
                <Textarea
                  id="edit-descripcion"
                  value={currentTema.descripcion}
                  onChange={(e) => setCurrentTema({ ...currentTema, descripcion: e.target.value })}
                />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="grid gap-2">
                  ...
                  <Select
                    value={currentTema.materiaId}
                    onValueChange={(value) => setCurrentTema({ ...currentTema, materiaId: value })}
                  >
                    <SelectTrigger id="edit-materia">
                      <SelectValue placeholder="Seleccionar materia" />
                    </SelectTrigger>
                    <SelectContent>
                      {teacherSubjects.map((subject) => (
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    value={String(currentTema.grado)}
                    onValueChange={(value) => setCurrentTema({ ...currentTema, grado: Number.parseInt(value) })}
                  >
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    value={currentTema.grupo}
                    onValueChange={(value) => setCurrentTema({ ...currentTema, grupo: value })}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Input
                    id="edit-fechaInicio"
                    type="date"
                    value={currentTema.fechaInicio}
                    onChange={(e) => setCurrentTema({ ...currentTema, fechaInicio: e.target.value })}
                  />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="edit-fechaFin">Fecha de Finalización</Label>
                  <Input
                    id="edit-fechaFin"
                    type="date"
                    value={currentTema.fechaFin}
                    onChange={(e) => setCurrentTema({ ...currentTema, fechaFin: e.target.value })}
                  />
                </div>
              </div>
              <div className="grid gap-2">
                ...
                <Textarea
                  id="edit-objetivos"
                  value={currentTema.objetivos}
                  onChange={(e) => setCurrentTema({ ...currentTema, objetivos: e.target.value })}
                />
              </div>
              <div className="grid gap-2">
                ...
                <Textarea
                  id="edit-contenido"
                  value={currentTema.contenido}
                  onChange={(e) => setCurrentTema({ ...currentTema, contenido: e.target.value })}
                />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="edit-actividades">Actividades</Label>
                <Textarea
                  id="edit-actividades"
                  value={currentTema.actividades}
                  onChange={(e) => setCurrentTema({ ...currentTema, actividades: e.target.value })}
                />
              </div>
              <div className="grid gap-2">
                ...
                <Textarea
                  id="edit-recursos"
                  value={currentTema.recursos}
                  onChange={(e) => setCurrentTema({ ...currentTema, recursos: e.target.value })}
                />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="edit-evaluacion">Evaluación</Label>
                <Textarea
                  id="edit-evaluacion"
                  value={currentTema.evaluacion}
                  onChange={(e) => setCurrentTema({ ...currentTema, evaluacion: e.target.value })}
                />
              </div>
            </div>
          )}
          <DialogFooter>
            ...
            <Button onClick={handleUpdateTema} className="bg-primary hover:bg-primary/90">
              Guardar Cambios
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div>
                <span className="font-semibold text-secondary">Fechas:</span>{" "}
